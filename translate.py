...
for record in records:
    ...
    if id_str not in output_data:
        ...
        if source_text.strip():
            try:
                # use bing to avoid google rate limit
                output_data[id_str] = ts.translate_text(source_text, translator='bing', from_language='en', to_language='gu')
            except Exception as e:
                print(f"Error on {id_str}: {e}")
                time.sleep(5)
                try:
                    output_data[id_str] = ts.translate_text(source_text, translator='google', from_language='en', to_language='gu')
                except Exception as e:
